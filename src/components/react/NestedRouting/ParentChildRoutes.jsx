...
function URLBar({ url }) {
  return (
    <div className="relative flex items-center px-1 md:px-2 md:pt-2">
...
function Header({ isHovering }) {
  return (
    <div className="relative bg-white/10 p-4 flex justify-end">
      <div className="flex gap-4">
        <div className="bg-white px-3 py-2 w-64 rounded text-slate-800 text-xs">
          Search
        </div>
      </div>
      <div
        style={{ marginTop: 0 }}
        className="flex items-center pr-4 pl-8 font-semibold text-slate-100"
      >
        Log in
      </div>
      {isHovering && (
        <div
          style={{ marginTop: 0 }}
          className="absolute grid content-center inset-0 bg-blue-400/10 border-4 border-blue-500 shadow shadow-black backdrop-blur-sm"
        >
...
function Body({ isHovering }) {
  return (
    <div
      style={{ marginTop: 0 }}
      className="relative flex items-center justify-center h-48 bg-black/10"
    >
      <div className="w-1/2">
      ...
      {isHovering && (
        <div
          style={{ marginTop: 0 }}
          className="absolute grid content-center inset-0 bg-teal-400/10 border-4 border-teal-500 shadow shadow-black backdrop-blur-sm"
        >
...
function Footer({ isHovering }) {
  return (
    <div style={{ marginTop: 0 }} className="relative bg-white/10 px-4">
      <div>
        <div className="flex justify-between border-b border-white px-8 py-4">
...
function Buttons({ handleHover }) {
  return (
    <div className="flex justify-around gap-4 pt-8 pb-6">
      <div
        onMouseEnter={() => handleHover("parent")}
        onMouseLeave={() => handleHover(null)}
        className="cursor-pointer inline-block text-xs font-semibold text-blue-400 px-8 py-4 rounded-3xl bg-blue-800"
      >
        {"<root.tsx>"}
      </div>
      <div
        ...
        onMouseLeave={() => handleHover(null)}
        style={{ marginTop: 0 }}
        className="cursor-pointer inline-block text-xs font-semibold text-teal-400 px-8 py-4 rounded-3xl bg-teal-800"
      >
        {"<_index.tsx>"}
      </div>
    </div>
...
export default function EpicNews() {
  ...
  return (
    <div className="mt-4 pt-4 bg-white/10">
