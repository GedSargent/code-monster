...
- Add a 'Publish' button to each article that changes the status to `published`.
- Update the code in the `/news` routes to only display articles that are `published`.
- Update the code in the `/article/$articleId` to display a new 'Published on' date if the article is published.
- Show a message to the user if they try to access an article that is not published.

#### EXTRA CREDIT 🚀 (optional)

Rather than simply display a message, if a user tries to access the `/article/$articleId` route for an article that is not published, redirect them to the `/news` page and display a toast notification informing them that the article is being reviewed.

### Hints

- Look at the layout of the `/users/$username/articles` page for ideas on how to lay this page out
- Think carefully how you will make the 'Publish' button work. Rememeber there are two fields to update: `isPublished` (boolean) and `publishedAt` (date).
- Once this functionality is in place, you should update the `/news` parent and child routes to only display articles that are published.
- You will need to update the `/article/$articleId` route to display the `publishedAt` field in a human-friendly format. You can find an example of date formatting in the `/users/$username/articles` page.
...
The first thing I will need is a new parent route that will display a list of all articles.

This is straightforward. In `/admin-review`, I'll just set up a `loader` function that returns the `id`, `title` and `isPublished` values for each article:
...
I can always customise this later if I want.

I'll also create a child index route that displays a message asking the user to select an article to review:

```tsx title="app/routes/admin-review/index.tsx"
...
<MonsterReveal title="2. Server-side code">
...
Remembering that adding a `.server` suffix to the file name will make it a server route, I'll create a new child route called `/__admin-review.$articleId.server.tsx`:
...
<MonsterReveal title="3. Create `/admin-review.$articleId.tsx`">

### 3. Create the `/admin-review/$articleId` route
...
Here is the full code for the `/admin-review/$articleId` route:
	...
	return (
		<div className="absolute inset-0 flex flex-col px-10">
			...
			<div className="mb-4 flex gap-4">
				...
				<p
					className={`w-fit rounded-lg ${data.article.isPublished ? 'bg-green-800' : 'bg-destructive'} px-4 py-2 text-sm text-card-foreground`}
				>
