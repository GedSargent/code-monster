...
This first step is surprisingly easy - we only need to add a single line of code!

Open `app/routes/admin-review.tsx` and add the code shown below:

```diff lang="tsx" title="app/routes/admin-review.tsx" ins="LoaderFunctionArgs, " ins="{ request }: LoaderFunctionArgs"
...
The `requireUserWithRole` function is already included in the Epic Stack. It firstly checks if a user is logged in and has the specified role. In this case, if the user does not have the 'admin' role, it will throw an unauthorised (403) error.
...
An `ErrorBoundary` is a React component that catches JavaScript errors (such as the 403 'Unauthorized' error shown here) anywhere in its child component tree and logs those errors, displaying a fallback UI instead of crashing the whole app.

Let's add an `ErrorBoundary` component to `app/routes/admin-review.tsx`.

First, import the `GeneralErrorBoundary` and `Button`
