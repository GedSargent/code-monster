...
```diff lang="tsx" title="app/root.tsx" collapse={1-6}
...
```diff lang="tsx" title="app/root.tsx" collapse={1-17}
import { useLoaderData } from 'react-router'
import { type Route } from './+types/root.ts'
import { type loader } from './__root.server.tsx'
import { GeneralErrorBoundary } from './components/error-boundary.tsx'
...
2. If you look inside the `HeroCallToAction` component code, you will see that it has something called an [**`interface`**](https://prismic.io/blog/typescript-interfaces) defined at the very top of the file.

		This interface describes the props that the component expects to receive.

		Let's take a look:

		```tsx title="app/components/organisms/Hero/HeroCallToAction.tsx" showLineNumbers {1-6} collapse={9-35}
...
3. You will notice that the `HeroCallToAction` component has a `hasBackgroundColour` prop, but this is not used anywhere inside the component.
		...
		- TIP: Look at how similar logic has been implemented with the `imageRight` prop.
...
4. Currently, the `HeroCallToAction` component has a lot of space around it from the title and horizontal margins.
	...
	![Hero Call To Action With Space](src/assets/webp/epic-news/hero-call-to-action-with-space.webp)
		...
		Can you modify the `root.tsx` file to:
			...
			- move the `<h1>` title at the top of the page so that it becomes part of the `HeroCallToAction` component instead?
...
```diff lang="tsx" title="app/root.tsx" collapse={1-8}
import { useLoaderData } from 'react-router'
import { type Route } from './+types/root.ts'
import { type loader } from './__root.server.tsx'
import { GeneralErrorBoundary } from './components/error-boundary.tsx'
...
Note that, to get the `Button` and `Link` components to render correctly, you will need to import them at the top of the file:

```diff lang="tsx" title="app/root.tsx" ins="Link, " collapse={4-10} showLineNumbers
...
You'll notice that the `className` of this `div` tag is using a [ternary operator](https://scrimba.com/advanced-javascript-c03kpi3kss/~0xse) to conditionally apply the `'md:flex-row-reverse'` class if `imageRight` is `true`.
...
Look carefully where the red highlighted code is located in your file, then replace it with the green highlighted code below:
...
1. Head back to `app/root.tsx`.

2. Remove the `<h1>` title from the `App` component, and the parent `div` wrapping the `HeroCallToAction` component:
...
You'll notice that, as we've been adding more `import`s to the top of `app/root.tsx`, a few of these have been underlined with yellow squiggly lines.

This is because ESLint wants us to organise our `import`s in a certain way.

![Organise imports](src/assets/webp/epic-news/custom-components/imports-need-organising.webp)
...
![Fix all auto-fixable problems](src/assets/webp/epic-news/custom-components/fix-all-imports.webp)

VS Code will automatically organise your `import`s for you, and the yellow underlines will disappear.

:::tip

Remember to do this every time an `import` is underlined with a yellow squiggly line. It's a good habit to get into!
