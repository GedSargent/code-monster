...
2. Next type or copy the following command in your terminal and hit enter:
...
The `react-scroll-parallax` package requires that our ENTIRE application is wrapped in a `ParallaxProvider` component. This allows the package to track the parallax effect across multiple areas of the app.

1. Firstly, open `app/root.tsx`.

2. Next, `import` the `ParallaxProvider` at the top of your `app/root.tsx` file:

```diff lang="tsx" title="app/root.tsx" collapse={1-9}
...
1. Open the `ParallaxBackground` component file first. Look carefully at its `interface` to see what props it accepts
...
```diff lang="tsx" title="app/root.tsx" showLineNumbers collapse={1-10}
import { Link, useLoaderData } from 'react-router'
import { type Route } from './+types/root.ts'
import { type loader } from './__root.server.tsx'
import { GeneralErrorBoundary } from './components/error-boundary.tsx'
...
```diff lang="tsx" title="app/root.tsx" showLineNumbers collapse={2-4, 15-29, 54-63}
export default function App() {
	const data = useLoaderData<typeof loader | null>()
	const theme = useTheme()
	const nonce = useNonce()
...
Start your server with `npm run dev` (if you need to) and view your site in the browser.

You should see the `ParallaxBackground` component displayed on the homepage, and a parallax effect applied as you scroll up and down:
...
There are lots of other custom components that I can share with you in OneNote.
...
Have you remembered to [tidy the imports](/code-monster/guides/epic-news/custom-components/hero-call-to-action#housekeeping) at the top of your `app/root.tsx` file?
...
- What built-in custom components have you used on your homepage? What modifications have you made and why?
- What other customisations have you made to the site's homepage?
- Have you built any custom components yourself?
