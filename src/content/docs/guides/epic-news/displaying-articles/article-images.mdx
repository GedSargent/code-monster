...
This tutorial guide is optional. Only complete this if you are looking for a challenge, or return to it as an optional extra at the end of your project if you have time.
...
1. Open `app/routes/news.$category.tsx`.
	...
	Adding the code below will initially produce a red underline again, but don't worry. Like earlier, we will fix this in the next steps.
...
2. Add the following import statements at the top of the `ArticleCard` component file to import the `getArticleImgSrc` function:
		...
		```diff lang="tsx" title="app/components/organisms/ArticleCard.tsx" showLineNumbers collapse={11-37}
		...
		+import siteLogo from '~/assets/svg/site-logo.svg' // !!! Change to your own logo!!!
...
4. With this in place, let's now modify the logic inside our `ArticleCard` component to display the article image above the title:
		...
		```diff lang="tsx" title="app/routes/news.$category.tsx" showLineNumbers collapse={1-9, 24-29}
		import { type ReactElement } from 'react'
		import {
			MdOutlineBusinessCenter,
			MdOutlineTheaters,
