...
Navigate down to the `ArticleRoute` component in the same file (around line 120), and add the JSX highlighted in green just below the `h2` element:
...
Although this is the same article data, it's actually slightly different. This is because we've now opened the article editor form, which is a separate route in the app.
