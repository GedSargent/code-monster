...
Find the `ProfileRoute` component (around line 31), and add the following code to it, looking carefully at the code below to see where it should be placed (just before the 'My Notes' button).
...
Navigate down to the `ArticleRoute` component in the same file (around line 108), and add the JSX highlighted in green just below the `h2` element:
...
These lines of code need adding, starting from around line 68:

```diff lang="tsx" title="app/routes/users+/$username_+/__article-editor.tsx" ins=" | 'categoryId'" showLineNumbers startLineNumber=66
...
Open the component file at `app/components/molecules/SelectorGroup.tsx` and take a look at the code.
