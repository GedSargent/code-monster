...
With our database schema and seed data in place, let's move on to creating articles.

Let's create some articles for our users to read.
...
Open the file at `app/routes/users+/$username.tsx`.

Find the `ProfileRoute` component (around line 31), and add the following code to it, looking carefully at the code below to see where it should be placed.

Feel free to expand the collapsed code sections to see the full component:

```diff lang="tsx" title="app/routes/users+/$username.tsx" showLineNumbers collapse={2-42, 61-73}
...
Save your changes, and head back to the browser. You should see the new button link to the 'Articles' page:

![Profile with articles link](src/assets/webp/epic-news/profile-with-articles-link.webp)

Click on this to navigate to the articles page.
...
Open `app/routes/users+/$username_+/articles.$articleId.tsx` now.

At the moment, the `article` being loaded does not contain any `category` data. Let's update this:
...
This is because we are adding new fields to the article object that are not yet defined in the form we are passing it to.

Don't worry about this for now, as we will update the form to include the new `category` field later.
...
Navigate down to the `ArticleRoute` component in the same file (around line 100), and add the JSX highlighted in green just below the `h2` element:
...
With this in place, save your changes and you should now see the `category` name displayed on the article page:
...
If a user clicks on an existing article they are the owner of (or have permissions to edit), they should be presented with an 'Edit' button. Clicking this button will take them to the article editor form, where they can update the article:
...
Although this looks like the same article, it's not. We've now opened the article editor form, which is a separate route in the app.

As such, Remix has loaded the article data again, but this time it's for the article editor form.

Open the file at `app/routes/users+/$username_+/articles.$articleId_.edit.tsx`, and update the code as follows:
...
Open the file at `app/routes/users+/$username_+/articles.new.tsx` and add the code below:
...
Open the file at `app/routes/users+/$username_+/__article-editor.tsx` now. This file handles the actual article editor form.
...
### Update the `ArticleEditorSchema`
...
Next, add these new values to the `ArticleEditorSchema` object, defined around line 54 to include the new `category` field:
...
The `ArticleEditorSchema` is a Zod schema object that defines the constraints for the form fields in the article editor form.

We are adding a new field to the schema called `categoryId`, which will hold the `id` of the selected category. This field is optional, as the user may not select a category.
...
We need to include to include a new `category` field to the `article`, and a new `categories` prop definition.

These need adding around line 62:

```diff lang="tsx" title="app/routes/users+/$username_+/__article-editor.tsx" ins=" | 'categoryId'"
...
Scroll down the `ArticleEditor` component further, and you will find the `TextAreaField` for the `content` field, around line 121.

Let's place our choices for the `category` field just below this:

```diff lang="tsx" title="app/routes/users+/$username_+/__article-editor.tsx" showLineNumbers collapse={2-12, 14-31, 42-59, 76-116, 118-130}
export function ArticleEditor({
	article,
	categories,
}: {
...
The `name` generates the keys of `FormData` object, and the `value` attribute generates the value of the respective key.
...
Open the component file at `app/components/molecules/SelectorGroup.tsx` now, and take a look at the code.

Add the code shown below:

```diff lang="tsx" title="app/components/molecules/SelectorGroup.tsx" showLineNumbers collapse={}
...
Go back to the code in `app/routes/users+/$username_+/__article-editor.tsx`, and add the following props:
...
Save your changes, and we can move on to the next step.

### Update `ArticleEditor.server.tsx`

When data is sent from a Remix `Form` component, it is sent to the nearest server-side `action` function which handles the form submission.
...
Luckily, we only need to make three minor changes to ensure we capture the new `categoryId` field from the form data:

```diff lang="tsx" title="app/routes/users+/$username_+/__article-editor.server.tsx" showLineNumbers collapse={}
import { parseWithZod } from '@conform-to/zod'
...
Aim for 10-15 new articles, each with different categories, and some with no category at all.
