...
function ArticleCard({ image, title, categoryTitle }: ArticleCardProps) {
	return (
		<div>
			<div className={`relative h-44 lg:h-52`}>
				<div className="absolute inset-0">
			...
			>
				...
				<div className="flex items-center gap-3">
					<img src={image} alt={categoryTitle} className="h-6 w-6" />
					<span className="text-sm text-muted-foreground">
						{categoryTitle}
					</span>
				</div>
...
As you can see, all the code inside the `return` statement of the `ArticleCard` component is the same as the code we highlighted above in the `NewsCategoryPage` component.

All that was needed was to cut and paste the code straight across, then swap out hardcoded values for the props that we are passing to the component.
...
+							key={article.id}
+							title={article.title}
+							categoryTitle={toTitleCase(article.category) || 'General News'}
+							image={article.image}
+						/>
+					)
+				})}
