...
2. Let's start by renaming `allArticles` to something more suitable:
		...
		-	return data({ category, allArticles })
...
6. ## Check filters in the browser
	...
	If you are seeing a blank page for some of the categories, try updating some of your user's articles so that they have a category assigned to them.

	We looked at editing article categories in the [Creating Articles tutorial](/code-monster/guides/epic-news/displaying-articles/creating-articles#display-categories)
...
1. Firstly, import the `prisma` client, the `data` helper and your `ArticleCard` component at the top of your file.

		```diff lang="tsx" title="app/routes/_index.tsx" ins="data, " ins=", useLoaderData"
...
2. Next, add a `loader` function between the `meta` function and your `Index` component.
		...
		```diff lang="tsx" title="app/routes/_index.tsx" showLineNumbers collapse={1-7, 25-47}
		import { data, Link, type MetaFunction, useLoaderData } from 'react-router'
		import ParallaxBackground from '#app/components/organisms/Hero/ParallaxBackground.tsx'
		import { Button } from '#app/components/ui/button.tsx'
		import heroImage from '~/assets/jpg/sample-hero.jpg'
...
3. Finally, add a section to populate with articles.
		...
		```diff lang="tsx" title="app/routes/_index.tsx" showLineNumbers collapse={1-22, 30-47}
		import { data, Link, type MetaFunction, useLoaderData } from 'react-router'
		import ParallaxBackground from '#app/components/organisms/Hero/ParallaxBackground.tsx'
		import { Button } from '#app/components/ui/button.tsx'
		import heroImage from '~/assets/jpg/sample-hero.jpg'
