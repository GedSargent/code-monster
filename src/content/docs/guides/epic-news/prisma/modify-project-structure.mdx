...
### 💡 Tip - open files quickly

Remember that you can quickly find a file inside VS Code by using the keyboard shortcut <MonsterKbd>CTRL + P</MonsterKbd> (Windows) or <MonsterKbd>CMD + P</MonsterKbd> (Mac) and typing the name of the file.
...
1. Open `app/routes/resources+/note-images.$imageId.tsx`.

2. Copy the entire file (<MonsterKbd>CTRL + C</MonsterKbd>) from the file browser on your left.

3. Make sure the same `resources+` folder is still open and selected in the file browser.

4. Right-click the `resources+` folder, and select 'Paste' to paste a copy of the file back into the same `resources+` folder

5. Rename your newly pasted file as `article-images.$imageId.tsx`:
