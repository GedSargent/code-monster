...
```diff lang="ts" title="prisma/seed.ts" showlineNumbers startLineNumber=1 ins="getArticleImages," ins="'article', 'articleCategory'" collapse={15-20, 27-73, 122-164, 254-289}
...
+			businessCategory.id,
+		]
+		const randomCategoryIdIndex = faker.number.int({
+			min: 0,
...
+							categoryId: categoryIds[randomCategoryIdIndex],
						})),
					},
				},
			})
