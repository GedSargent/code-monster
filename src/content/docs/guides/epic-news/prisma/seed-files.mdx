...
Quite often in development, we want to quickly generate some data for our database. For example, we might want to add some test users, or some categories for our articles.

A seed file is the perfect way to do this.

In Prisma, the seed file is located in `prisma/seed.ts`. Open this now in your code editor.

### Adding categories.

Inside `prisma/seed.js` file, add the following code at around line 252. Look carefully at the code around it to understand where it should go:

```diff lang="ts" title="prisma/seed.ts" showLineNumbers collapse={1-11, 14-249}
...
You should see a list of article categories in the database:

![Prisma Studio with article categories](src/assets/webp/epic-news/prisma/article-categories.webp)
