...
As you can see, the first mock article, found at index `0` of the `mockArticles` array, has several properties. For example, it has
...
Having to write out the `firstArticle` variable and then access its properties is a bit verbose. We can simplify this by using object destructuring.
...
2. After destructuring each property, replace all the `firstArticle` references with direct property names.
    ...
    For example, instead of writing `<h3>{firstArticle.title}</h3>`, you can write `<h3>{title}</h3>`. Or instead of `<img src={firstArticle.image} alt={firstArticle.title} />`, you can write `<img src={image} alt={title} />`.
