...
Creating static files for each child route is fine, but not very sustainable. 

Think about this for a second.

What if we want to add new categories? We'd have to create a new child file for each one, and then update the code inside each so they are all consistent.

There's a much better solution - creating a single ***dynamic route*** that will handle requests for all the different news categories.
...
![Delete child routes](src/assets/gif/delete-individual-child-routes.gif)
