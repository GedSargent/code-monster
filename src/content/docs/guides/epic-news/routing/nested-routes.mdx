...
- The file name must match the route URL, e.g.
	...
	- Spelling, case and special characters are important.
			...
			If we named the file `AboutUs.tsx`, then the URL would be `/AboutUs`, which is not what we want for the URL in a web app.
...
4. Save the file again and head back to your browser.
		...
		Just like we did in `root.tsx`, by adding an `Outlet` component to the `news.tsx` file, we have converted it into a parent route that can render child routes.
...
The `NavLink` component is a special Remix `<Link>` that knows whether it is **"active"** or not. It does this via a special `isActive` boolean that Remix supplies for us.
...
With this code in place, refresh your page.
