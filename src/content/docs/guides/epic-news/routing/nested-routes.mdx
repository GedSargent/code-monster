...
So in effect, this means that the `root.tsx` file is actually the **parent** for ***every*** other route in the app. Any code we add here will be globally applied to all the other routes across the site.

But what if we could add more fine-grained parent and child routes to a Remix app? How could we show Remix know which files are ***parents***, which are ***siblings*** and which are their ***children***?
...
- app
	...
	- routes
		- _index.tsx // child/sibling route
		- about-us.tsx // child/sibling route
		- contact.tsx // child/sibling route
...
### Understanding file names and URL conventions
