...
But what does it mean to say this? How does it map exactly?

Let's take a look at the file structure of the Epic News app to see how it relates to the URL structure.

## The "root" route and the base URL

`root.tsx`, and by extension `_index.tsx` (once we add the `Outlet` component to `root.tsx`), maps to `/`, or the base URL of the app - `http://localhost:3000/` in our case.
...
But how does Remix know which files are related to one another? How does it know which files are ***siblings*** and which are ***children***?
...
1. Using your knowledge of parent/child routes and the `NavLink` component, add links to the other news categories in the `NewsPage` component:
...
Using what you have learned about the `_index.tsx` special filename, can you work out how to diplay the text below when someone visits the `http://localhost:3000/news` URL?
...
We saw, in the extension challenge above, how to use the `_index.tsx` naming convention to render a welcome message when the user visits the parent route URL.
