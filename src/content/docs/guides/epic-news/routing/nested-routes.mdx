...
This is because the `root.tsx` file is still rendering the code for the header and footer, but the `Outlet` component we placed between these elements [**earlier**](/code-monster/guides/epic-news/routing/outlet#the-outlet-component) is now rendering the `NewsPage` component as the child route.
