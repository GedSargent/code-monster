...
Code highlighted in green should be **added** to your files, and code highlighted in red should be **removed**.
...
You should see the welcome message has changed:
...
This is quite a strange concept to understand at first, so let's take some time to understand what it means.
...
The reason we can still see the navbar and footer is because this is code being served from the **parent** `root.tsx` file. However, the content that we see in the middle of the page is now being served from the `_index.tsx` file through the `Outlet` component.
