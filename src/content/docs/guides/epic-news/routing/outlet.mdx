...
But how does Remix know which route files are related? How does it know which should be "children" of the parent? The answer lies in Remix's file naming conventions.
...
3. Make sure the navbar, footer and theme switcher are still present on each page.
...
In the next lesson, we will learn how to create sibling routes with their own children. This will allow us to create a more complex route structure that can serve multiple pages from the same level in the app.
