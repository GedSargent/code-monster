...
2. Comment out the all your code that is currently between the `<main>` tags in the `root.tsx` file (highlighted in white below - your actual code might be different here)

3. Add the `Outlet` component beneath the commented out code

```diff lang="tsx" title="app/root.tsx" ins="Outlet, " showLineNumbers collapse={5-17, 56-62} {31-52}
...
In a Remix app, this file would typically be found inside the `routes` folder, and called `about-us.tsx`.
...
As soon as we place the `Outlet` component inside a route file (like we just have in `root.tsx`), it becomes a "parent" route.
...
Add a new route to your app that serve pages from the URL paths below. For the time being, you can just add a simple title to each page in the same way we did for `/about-us`.

The new route URL should be `/contact-us`.

You should also add a link to the new page in your navbar, and add the `prefetch` attribute to the link to speed up page loading times.
...
3. Update your navbar and footer to include links to these new pages.
4. Add a `prefetch` attribute to these new links to make them load faster in the browser.
