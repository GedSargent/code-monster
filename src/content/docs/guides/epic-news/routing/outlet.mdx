...
As you'll see, the `root.tsx` file is actually the springboard from which we can render a _shared layout_ that all of our other pages depend on.
...
1. Add `Outlet` as an extra import at the top of the `root.tsx` file:
...
2. Comment out the all your code that is currently wrapped between the `div` with a `className` of `flex-1`. 
		...
		This will include the opening and closing `<main>` tag, along with all its children.
		...
		```diff lang="tsx" title="app/root.tsx" showLineNumbers collapse={1-21} {35-56}
		import { Link, Outlet, useLoaderData } from 'react-router'
		import { ParallaxProvider } from 'react-scroll-parallax'
		import { type Route } from './+types/root.ts'
		import { type loader } from './__root.server.tsx'
...
3. Add the `Outlet` component beneath the commented out code
		...
		```diff lang="tsx" title="app/root.tsx" showLineNumbers collapse={1-17, 32-51}
...
Well - the screenshot itself provides a big clue to this last question! The code for the main page content is now coming from a completely different file to the one we have been working on!

Specifically, we are now looking at content from the `app/routes/_index.tsx` file.

Open this file at `app/routes/_index.tsx` and you'll find the matching code:
...
So what exactly is happening here? Why is the `Outlet` component suddenly rendering `_index.tsx`?

To understand this, we need to take a brief detour into the world of "routes".

::::note[Deep Dive]

### What is a "route"? 🔀

In a web app, a "route" is a web address (URL) that shows a page or view in the app. Every route is served from a specific file in the code base.
...
As soon as we place the `Outlet` component inside a route file (like we just have in `root.tsx`), it converts the file into what we call a "parent" route.

The "parent" calls another file, which becomes the "child" route, via the `Outlet` component.

`Outlet` tells Remix **where** to render the matching child route file within the parent's content.
...
So, how does Remix know which route file to load as "children" of its "parent"? 🤔
...
This is because they are still being rendered from `root.tsx`, but the main page content is now being rendered from `about-us.tsx`.
...
Let's update the navbar to include a link to the new 'About us' page.
