...
Add links to three new categories that will feature in your `news` route.

**Remember** - you don't need to create a new file for each category now. Just add a new `Link` to each one in your `app/routes/news.tsx` file.

There's no solution for this challenge! You should be able to figure out this one for yourself 😅.
