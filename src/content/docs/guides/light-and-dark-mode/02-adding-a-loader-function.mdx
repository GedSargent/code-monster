...
```diff lang="tsx" title="app/root.tsx" ins="LoaderFunctionArgs, json,"
...
If a `loader` function is exported from a **route** file (i.e. a React file inside the `route` folder), then Remix will execute this code on the **server** _before_ the user interface (UI) is rendered. This ensures that any required data is accessed and processed before the UI is presented by the browser.
