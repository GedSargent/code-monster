...
To upload images to Cloudinary, we need to provide our Cloudinary credentials. These credentials will be stored in environment variables.

:::note[Cloudinary signup]
You should have already signed up to Cloudinary for a free account in a [previous guide](/code-monster/guides/prisma/adding-an-image-to-the-game-model#step-4-upload-images-to-cloudinary). If you have not done so, please do so now.
:::

Open the `.env` file in the root of your project and add the following variables beneath the ones already there:
...
:::note[Where do I find these credentials?]
You can find your Cloudinary credentials in the Cloudinary dashboard.

<Steps>
1. Log in to your Cloudinary account
2. Click on the **Account** tab
3. Click on the **Environment Variables** tab
4. Copy the values for `Cloud name`, `API Key`, and `API Secret`
5. Paste these values into your `.env` file, replacing the `your_` placeholders with the actual values
</Steps>
:::
