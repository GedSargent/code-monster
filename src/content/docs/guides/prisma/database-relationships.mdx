...
We will start by creating a new table called `Article` in the `schema.prisma` file. This table will have a `title` and a `content` field, as well as a `createdAt` and `updatedAt` field to keep track of when the article was created and last updated.
