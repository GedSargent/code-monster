...
We are going to set up a database relationship using Prisma. We will create a new table called `Article` and link it to the `User` table.
...
We will start by creating a new table called `Article` in the `schema.prisma` file. This table will have a `title` and a `content` field, as well as a `createdAt` and `updatedAt` field to keep track of when the article was created and last updated._createMdxContent
...
This adds a new `Article` model which shares several similar fields to the `User` model, but differs in the `title` and `content` fields.

At the moment, the two tables are completely unaware of each other in the database. We need to change that.

### Link the `Article` and `User` tables

If every `Article` will belong to a single `User`, which fields do you think we can use to link the `Article` and `User` tables together? 🤔

***HINT**: Remember that each user can have multiple articles associated with them, and each article must belong to a user.*

<MonsterDetails>
  <div>
    <p>We can use the `id` field of the `User` table to link it to the `Article` table.</p>
...
1. Firstly, we will add a new field to the `Article` model called `authorId`. This field will be of type `String` and will store the `id` of the `User` who created the article.
2. Next, we will add another new field called `author` to the same model. This field will be of type `User` and will link the `Article` and `User` tables together. We will add a `@relation` attribute to the `author` field to specify the fields that the relationship is based on.

Make the changes shown below to the `Article` model in the `schema.prisma` file to implement the relationship.
...
The red squiggly line under the `author` field indicates that the `User` model doesn't yet share a relation field with the `Article` model.
...
- Refresh your memory of how to generate a migration file by reviewing the [**previous lesson**](http://localhost:4321/code-monster/guides/prisma/prisma-schema#creating-a-migration-file).
...
<MonsterDetails>
  <div>
    <p>Running the command below in your terminal window will generate the file you need:</p>
    ```bash
    npx prisma migrate dev --name add-article-model
    ```
    <p>Don't worry if you see the warning below:</p>
    ![Article migration warning](src/assets/png/prisma/article-migration-warning.png)
    <p>We don't have any data in the database yet, so can ignore this. Just hit <MonsterKbd>y</MonsterKbd> and continue 😎</p>
  </div>
</MonsterDetails>
...
![Prisma Studio with Article table](src/assets/png/prisma/prisma-studio-article-table.png)
