...
To stop this from happening, we would usually add the `.env` file to the `.gitignore` file in the root of the project. This file contains a list of files and folders thats Git ignore when making commits.

![.gitignore file](src/assets/png/prisma/gitignore-file.png)

#### But where is the .env file here?

Because "Epic News" is an educational project and there is no sensitive information that will be added, we've removed the `.env` file from `.gitignore` to prevent any confusion during the initial setup stage.
