...
```diff lang="tsx" title="src/routes/articles.tsx"
...
If you check the browser now, you will see that the page title is now "MyBrilliantArticles":

![MyBrilliantArticles](src/assets/png/prisma/my-brilliant-articles.png)

### Arrays of React components

If we take this a step further, what do you think will happen if we replace the plain strings inside this array with React components?
...
+					{[
+						<span className="border border-red-500 p-2 text-red-500">My</span>,
+						<span className="border border-green-500 p-2 text-green-500">
+							Brilliant
+						</span>,
+						<span className="border border-blue-500 p-2 text-blue-500">
+							Articles
+						</span>,
+					]}
				</h1>
...
With this knowledge in mind, let's think how we can use the `map` function to dynamically render a list of `articles`.
...
2. The `map` function produces a new array from an existing one by applying a function to each element.

:::tip[Aha!]

<p class="text-center text-8xl">💡</p>

This means that we can use the `map` function to loop through an array of items and return a ***new array of components***.
