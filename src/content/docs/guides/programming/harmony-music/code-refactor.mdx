...
One way to solve this problem is to refactor your code. Refactoring is the process of restructuring your code without changing its external behavior.
...
6. Next, let's separate the logic further.
    ...
    ```diff lang="js" title="validateForm.js" showLineNumbers collapse={5-83}
    ...
    ```diff lang="js" title="validateForm.js" showLineNumbers collapse={29-77}
    export function displayErrors(errors) {
    +  for (let field in errors) {
    +    let inputElement = document.querySelector(`#${field}`);
    +    let labelElement = document.querySelector(`label[for=${field}]`);
...
7. Now, let's call the new `displayErrors` function if any errors exist:

    ```diff lang="js" title="validateForm.js" showLineNumbers collapse={2-25, 29-77}
    export function displayErrors(errors) {
      for (let field in errors) {
        let inputElement = document.querySelector(`#${field}`);
        let labelElement = document.querySelector(`label[for=${field}]`);
...
8. Our final step is to return the valid data from our function in an object that becomes the 'single source of truth' for the rest of our application code.
    ...
    ```diff lang="js" title="validateForm.js" showLineNumbers collapse={2-56}
    export function validateForm({ userEmail, userLevel, userHours }) {
      const maxHoursPerLevel = {
        basic: 5,
        advanced: 10,
