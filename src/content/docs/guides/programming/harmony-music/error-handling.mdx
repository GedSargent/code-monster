...
1. Firstly, let's add an object to collect all the errors into, together with a helper function to add error messages:
    ...
    ```diff lang="js" title="index.js" showLineNumbers collapse={6-17, 30-60}
...
2. Next, we will replace all the `alert` messages with objects that we will `push` into the `errors` array.
    ...
    -    return;
      ...
      // Check if the user has specified at least one hour of study
      if (userHours === "") {
    -    alert("Please specify how many hours of tuition you would like.");
    +    addError(
    +      "hoursPerWeek",
...
3. Let's test the new error handling by submitting the form with some errors.
    ...
    Fill in the form with some errors and submit it. You should see the errors collected in the console.

    ![Errors collected in the console](/img/docs/guides/programming/harmony-music/error-handling/errors-collected-in-console.png)

    The errors are now collected in an object, which we can use to display them to the user.
...
1. The first thing we should do is style the input fields that have errors.
    ...
    Open `index.css` and add the following CSS towards the bottom of your file:
...
2. Open `index.js`. Add the code highlighted below just above the last two `console.log()` statements:
...
To do this, we will need to update the HTML to include a `div` element for each error message, and then use JavaScript to populate these elements with the error messages.
...
3. Next, let's add the JavaScript that will add this new class to any areas of the form that have errors:
    ...
    ```diff lang="js" title="index.js" showLineNumbers collapse={7-76}
      // Capture user's input on form submission
      let form = document.querySelector("form");
...
4. Finally, we need to `return` early if there are any messages inside the `errors` object:
    ...
    ```diff lang="js" title="index.js" showLineNumbers collapse={7-91}
      // Capture user's input on form submission
      let form = document.querySelector("form");
