...
If you remember from our [previous lesson](/code-monster/guides/light-and-dark-mode/04-action-functions), `action` functions are only executed on the the **server**, and can only be placed directly on `route` components (i.e. `tsx` files stored in the `routes` folder). They are used to perform an action when a user interacts with the UI, such as clicking a button or submitting a form.
...
Using Remix's naming convention that we looked at together in the ['Nested Routes'](/code-monster/guides/nested-routes) guide, can you locate and open the project file that serves the page at `http://localhost:3000/users/<user-name>/notes/new`?
